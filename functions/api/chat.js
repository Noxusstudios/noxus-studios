...
export async function onRequestPost(context) {
  ...
  if (!Array.isArray(messages) || messages.length === 0) {
    return jsonResponse({ error: 'messages array required' }, 400, request);
  }

  if (messages.length > 30) {
    return jsonResponse(
      { error: 'Conversation got long — for anything deeper, please book a call with Joseph: https://calendly.com/noxusstudios/30min' },
      400,
      request,
    );
  }
  ...
  for (const m of messages) {
    if (!m || (m.role !== 'user' && m.role !== 'assistant') || typeof m.content !== 'string' || m.content.length > 4000) {
      return jsonResponse({ error: 'Invalid message format' }, 400, request);
    }
  }
  ...
  try {
    ...
    return jsonResponse({ error: 'Something went wrong on our end. Try again in a moment.' }, 500, request);
  }
}
